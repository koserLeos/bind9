...
client_send(ns_client_t *client) {
	 ...
	 */
	truncate = (client->message->flags & DNS_MESSAGEFLAG_TC);
		 ...
		 dns_rdataset_t **opt)
{
 ...
 no_nsid:
	...
	if (TCP_CLIENT(client) && USEKEEPALIVE(client)) {
		...
		isc_buffer_putuint16(&buf, (uint16_t) ns_g_advertisedtimo);

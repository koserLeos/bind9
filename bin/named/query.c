...
query_find(ns_client_t *client, dns_fetchevent_t *event, dns_rdatatype_t qtype)
{
	...
	if (RECURSIONOK(client)) {
		     ...
		     ((client->message->flags & DNS_MESSAGEFLAG_CD) == 0)))
		{
			if (isc_log_wouldlog(ns_g_lctx, ISC_LOG_DEBUG(1))) {
				...
				dns_rdatatype_format(qtype, typebuf,
						     sizeof(typebuf));
				ns_client_log(client, NS_LOGCATEGORY_CLIENT,
					      NS_LOGMODULE_QUERY,
					      ISC_LOG_DEBUG(1),
 ...
 restart:
	 ...
	 */
	if (!TCP_CLIENT(client) && client->view->requireservercookie &&
 ...
 db_find:
	...
	block_clientinfo = false;
	dns_ecs_init(&client->prefetch_ecs);
	if (!is_zone) {
		uint8_t ecsbits4, ecsbits6;

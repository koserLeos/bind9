...
get_rsasha256_key_ids_from_sigs() {
	zone=$1
...
zone=prepub
# Generate keys.
ksk=$("$KEYGEN" -K signer -f KSK -q -a RSASHA256 -b 2048 -n zone "$zone")
zsk1=$("$KEYGEN" -K signer -q -a RSASHA256 -b 2048 -n zone "$zone")
zsk2=$("$KEYGEN" -K signer -q -a RSASHA256 -b 2048 -n zone "$zone")
...
get_rsasha256_key_ids_from_sigs $zone | grep "^$zskid1$" > /dev/null || ret=1
get_rsasha256_key_ids_from_sigs $zone | grep "^$zskid2$" > /dev/null && ret=1
n=$((n+1))
...
ret=0
zone=example
key1=$($KEYGEN -K signer -q -a RSASHA256 -b 2048 -n zone $zone)
key2=$($KEYGEN -K signer -q -f KSK -a RSASHA256 -b 2048 -n zone $zone)
(
cd signer || exit 1
...
grep "SOA 8 1 50" signer/example.db.after > /dev/null || ret=1
n=$((n+1))
test "$ret" -eq 0 || echo_i "failed"
status=$((status+ret))
...
ret=0
zone=example
key1=$($KEYGEN -K signer -q -f KSK -a RSASHA256 -b 2048 -n zone $zone)
key2=$($KEYGEN -K signer -q -a RSASHA256 -b 2048 -n zone $zone)
keyid2=$(keyfile_to_key_id "$key2")
key3=$($KEYGEN -K signer -q -a RSASHA256 -b 2048 -n zone $zone)
...
get_rsasha256_key_ids_from_sigs $zone | grep "^$keyid2$" > /dev/null || ret=1
get_rsasha256_key_ids_from_sigs $zone | grep "^$keyid3$" > /dev/null || ret=1
n=$((n+1))
test "$ret" -eq 0 || echo_i "failed"
status=$((status+ret))
...
(
cd ns3 || exit 1
kskname=$($KEYGEN -q -3 -a RSASHA256 -fk update-nsec3.example)
(
...
grep -q "Done signing with key .*/RSASHA256" signing.out || {
        ret=1
        sed 's/^/ns3 /' signing.out | cat_i
}
n=$((n+1))
...
ret=0
key=$($KEYGEN -a RSASHA256 -q example.) || ret=1
mv "$key.key" "$key"
...
(
cd signer || exit 1
$KEYGEN -q -a RSASHA256 -3 -fK remove > /dev/null
$KEYGEN -q -a RSASHA256 -33 remove > /dev/null

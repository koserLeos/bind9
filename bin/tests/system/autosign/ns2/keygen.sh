...
kskname=`$KEYGEN -a RSASHA256 -3 -q -fk $zone`
$KEYGEN -a RSASHA256 -3 -q $zone > /dev/null
...
zonefile="${zone}.db"
infile="${zonefile}.in"
ksk=`$KEYGEN -a RSASHA256 -3 -q -fk $zone`
$KEYGEN -a RSASHA256 -3 -q $zone > /dev/null
...
then
   ...
   infile="${zonefile}.in"
   cat $infile > $zonefile
   for i in Xbar.+005+30676.key Xbar.+005+30804.key \
	    Xbar.+005+30676.private Xbar.+005+30804.private
   do

...
# active zsk
czsk1=`$KEYGEN -q -a rsasha256 -L 30 $czone`

# not yet published or active
czsk2=`$KEYGEN -q -a rsasha256 -P none -A none $czone`

# published but not active
czsk3=`$KEYGEN -q -a rsasha256 -A none $czone`

# inactive
czsk4=`$KEYGEN -q -a rsasha256 -P now-24h -A now-24h -I now $czone`

# active in 12 hours, inactive 12 hours after that...
czsk5=`$KEYGEN -q -a rsasha256 -P now+12h -A now+12h -I now+24h $czone`
...
# active ksk
cksk1=`$KEYGEN -q -a rsasha256 -fk -L 30 $czone`

# published but not YET active; will be active in 20 seconds
cksk2=`$KEYGEN -q -a rsasha256 -fk $czone`
# $SETTIME moved after other $KEYGENs

echo_i "revoking key"
# revoking key changes its ID
cksk3=`$KEYGEN -q -a rsasha256 -fk $czone`
cksk4=`$REVOKE $cksk3`

echo_i "setting up sync key"
cksk5=`$KEYGEN -q -a rsasha256 -fk -P now+1mo -A now+1mo -Psync now $czone`

echo_i "and future sync key"
cksk6=`$KEYGEN -q -a rsasha256 -fk -P now+1mo -A now+1mo -Psync now+1mo $czone`

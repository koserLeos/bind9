...
main() {
	...
	if (!EVP_DigestInit_ex(evp_md_ctx, EVP_sha1(), NULL) ||
	    !EVP_DigestUpdate(evp_md_ctx, "test", 4) ||
	    !EVP_SignFinal(evp_md_ctx, buf, &siglen, pkey))
	{
		return (1);
	}
	bytes = siglen;
	...
	if (!EVP_DigestInit_ex(evp_md_ctx, EVP_sha256(), NULL) ||
	    !EVP_DigestUpdate(evp_md_ctx, "test", 4) ||
	    !EVP_SignFinal(evp_md_ctx, buf, &siglen, pkey))
	{
		return (1);
	...
	if (!EVP_DigestInit_ex(evp_md_ctx, EVP_sha512(), NULL) ||
	    !EVP_DigestUpdate(evp_md_ctx, "test", 4) ||
	    !EVP_SignFinal(evp_md_ctx, buf, &siglen, pkey))
	{
		return (1);

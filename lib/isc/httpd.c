...
#define BUFLENOK(s) (httpd->recvbuf - (s) < (int)HTTP_RECVLEN)
...
/*
 * The \r\n are actually not valid in the single header value, but we are going
 * to skip them anyway.
 */
#define IS_VALUE_DELIMITER(c) (isspace(c) || c == ',')
...
value_match(const struct phr_header *header, const char *match) {
	...
	for (size_t i = 0; i < limit; i++) {
		if (IS_VALUE_DELIMITER(header->value[i])) {
			while (i < limit &&
			       IS_VALUE_DELIMITER(header->value[i])) {
				i++;
			}
			continue;
		}
		...
		if (strncasecmp(&header->value[i], match, match_len) == 0) {
			 ...
			 * 'deflate[[:space:],;]', but not 'deflateyou'
			 */

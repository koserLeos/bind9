...
isc_netmgr_create(isc_mem_t *mctx, isc_loopmgr_t *loopmgr, isc_nm_t **netmgrp) {
	...
	netmgr = isc_mem_get(mctx, 1, sizeof(*netmgr));
	*netmgr = (isc_nm_t){
		.loopmgr = loopmgr,
	...
	netmgr->workers = isc_mem_get(mctx, netmgr->nloops,
...
nm_destroy(isc_nm_t **mgr0) {
	...
	*mgr0 = NULL;
	...
	isc_mem_put(mgr->mctx, mgr->workers, mgr->nloops,
...
nmsocket_cleanup(void *arg) {
	...
	if (sock->parent == NULL && sock->children != NULL) {
		 ...
		 * Now free them.
		 */
		isc_mem_put(sock->worker->mctx, sock->children, sock->nchildren,
			    sizeof(*sock));
		sock->children = NULL;
		sock->nchildren = 0;
	}
	...
	if (sock->parent == NULL) {
		...
		isc_mem_put(worker->mctx, sock, 1, sizeof(*sock));
	}
...
alloc_handle(isc_nmsocket_t *sock) {
	isc_nmhandle_t *handle = isc_mem_get(sock->worker->mctx, 1,
...
nmhandle_free(isc_nmsocket_t *sock, isc_nmhandle_t *handle) {
	...
	isc_mem_put(sock->worker->mctx, handle, 1, sizeof(*handle));
}
...
isc__networker_destroy(isc__networker_t *worker) {
	...
	isc_mem_putanddetach(&worker->mctx, worker->recvbuf,
			     ISC_NETMGR_RECVBUF_SIZE, sizeof(char));

...
#ifdef USE_RA_SOCKET
static isc_result_t
ra_socket(uv_os_sock_t *fdp) {
	isc_result_t result;
	uv_os_sock_t fd = -1;
#if defined(ICMP6_FILTER) && defined(ICMP6_FILTER_SETBLOCKALL)
	struct icmp6_filter icmp6_filter;
#endif

	result = isc__nm_socket(AF_INET6, SOCK_DGRAM, IPPROTO_ICMPV6, &fd);
	if (result != ISC_R_SUCCESS) {
		result = isc__nm_socket(AF_INET6, SOCK_RAW, IPPROTO_ICMPV6,
					&fd);
		if (result != ISC_R_SUCCESS) {
			return (result);
		}
	...
	ICMP6_FILTER_SETPASS(ND_ROUTER_ADVERT, &icmp6_filter);

	setsockopt(fd, IPPROTO_ICMPV6, ICMP6_FILTER, &icmp6_filter,
		   sizeof(icmp6_filter));
#endif
...
static isc_result_t
ra_connect_direct(isc_nmsocket_t *sock) {
...
/*
 * Asynchronous 'raconnect' call handler: open a new 'udp' socket and
...
isc_nm_raconnect(isc_nm_t *mgr, isc_nm_cb_t cb, void *cbarg) {
	...
	if (isc__nm_in_netthread()) {
		atomic_store(&sock->active, true);
		sock->tid = isc_nm_tid();
		isc__nm_async_raconnect(&mgr->workers[sock->tid],
...
udp_recv_cb(uv_udp_t *handle, ssize_t nrecv, const uv_buf_t *buf,
	...
	if (!sock->route_sock && !sock->ra_sock) {

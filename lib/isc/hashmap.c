...
static void
hashmap_rehash_one(isc_hashmap_t *hashmap) {
	uint8_t idx = hashmap_nexttable(hashmap->hindex);
	uint32_t oldsize = hashmap->tables[idx].size;
	hashmap_node_t *oldtable = hashmap->tables[idx].table;
	hashmap_node_t node;
	isc_result_t result;
...
hashmap_add(isc_hashmap_t *hashmap, const uint32_t hashval, const uint8_t *key,
	...
	while (true) {
		...
		INSIST(psl < hashmap->tables[idx].size);
	}

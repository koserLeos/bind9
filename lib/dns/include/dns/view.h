...
struct dns_view {
	...
	dns_rrl_t **			rrls;
	unsigned int			rrlcount;
	dns_rbt_t *			rrldomains;
	bool				provideixfr;
	bool				requestnsid;
	bool				sendcookie;
	...
	uint8_t				ecsbits4;	/* Default ECS
							 * prefix len IPv4 */
	uint8_t				ecsbits6;	/* Default ECS
							 * prefix len IPv6 */

	dns_acl_t			*ecsforward;
	bool				ecscache;	/* Cache ECS? */
	bool				ecsprivacy;	/* Disable ECS when
							 * forwarding */
	 ...
	 * set.
	 */
	uint8_t				protoss_opts;
	uint32_t			protoss_va;
	uint32_t			protoss_org;
	uint64_t			protoss_dev;
};

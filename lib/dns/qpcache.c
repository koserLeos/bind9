...
struct qpdata {
	...
	/*%
	 * Used for deadnodes.  This linked list is used to mark nodes which
	 * have no data any longer, but we cannot unlink at that exact moment
	 * because we didn't have a write transaction open for the tree.
	 */
...
static void
clean_cache_node(qpcache_t *qpdb, qpdata_t *node) {
 ...
 */
static void
newref(qpcache_t *qpdb, qpdata_t *node,
 ...
 */
static bool
decref(qpcache_t *qpdb, qpdata_t *node, isc_rwlocktype_t *nlocktypep,
...
expireheader(dns_slabheader_t *header, dbmod_t *modctx,
	...
	if (isc_refcount_current(&HEADERNODE(header)->erefs) == 0) {
		 ...
		 */
		newref(qpdb, HEADERNODE(header), nlocktype DNS__DB_FLARG_PASS);
		decref(qpdb, HEADERNODE(header), &nlocktype,
...
static void
bindrdataset(qpcache_t *qpdb, qpdata_t *node, dns_slabheader_t *header,
...
static isc_result_t
find_deepest_zonecut(search_t *search, qpdata_t *node, dns_dbnode_t **nodep,
...
tree_exit:
	...
	if (search.need_cleanup) {
		...
		NODE_RDLOCK(lock, &nlocktype);
		decref(search.qpdb, node, &nlocktype, NULL DNS__DB_FLARG_PASS);
		NODE_UNLOCK(lock, &nlocktype);
	}
...
expire_lru_headers(qpcache_t *qpdb, unsigned int locknum, dbmod_t *modctx,
	     ...
	     header = ISC_LIST_TAIL(qpdb->lru[locknum]))
	{
		...
		expireheader(header, modctx, dns_expire_lru DNS__DB_FLARG_PASS);
		purged += header_size;
	}
...
again:
	do {
		...
		purged += expire_lru_headers(qpdb, locknum, modctx,
					     purgesize -
...
free_qpdb_rcu(struct rcu_head *rcu_head) {
	...
	if (qpdb->deadnodes != NULL) {
		...
		isc_mem_cput(qpdb->common.mctx, qpdb->deadnodes,
			     qpdb->node_lock_count, sizeof(qpdatalist_t));
	}
	/*
 ...
 *
 * The caller must hold bucketnum'th node (write) lock and pass a write
 * transaction.
 */
static void
cleanup_dead_nodes(qpcache_t *qpdb, dbmod_t *modctx,
		   int bucketnum DNS__DB_FLARG) {
	qpdata_t *node = NULL;
	...
	while (node != NULL && count > 0) {
		...
		/*
		 * We might have reactivated this node without a write
		 * transaction, so we couldn't remove this node from
		 * deadnodes then and we have to do it now.
		 */
		if (isc_refcount_current(&node->references) != 0 ||
		    node->data != NULL)
		{
 ...
 */
static void
reactivate_node(qpcache_t *qpdb, qpdata_t *node,
		dbmod_t *modctx DNS__DB_FLARG) {
	isc_rwlocktype_t nlocktype = isc_rwlocktype_none;
	 ...
	 */
	if (!ISC_LIST_EMPTY(qpdb->deadnodes[node->locknum]) && modctx != NULL &&
	    modctx->writing)
	{
		maybe_cleanup = true;
	}
	...
	if (ISC_LINK_LINKED(node, deadlink) || maybe_cleanup) {
		...
		if (maybe_cleanup) {
			cleanup_dead_nodes(qpdb, modctx,
...
static qpdata_t *
new_qpdata(qpcache_t *qpdb, const dns_name_t *name) {
...
detachnode(dns_db_t *db, dns_dbnode_t **targetp DNS__DB_FLARG) {
	...
	if (decref(qpdb, node, &nlocktype, NULL DNS__DB_FLARG_PASS)) {
...
static isc_result_t
add(qpcache_t *qpdb, qpdata_t *qpnode,
...
addrdataset(dns_db_t *db, dns_dbnode_t *node,
	...
	*newheader = (dns_slabheader_t){
	...
	expire_ttl_headers(qpdb, qpnode->locknum, &modctx, now,
...
dereference_iter_node(qpdb_dbiterator_t *qpdbiter DNS__DB_FLARG) {
	...
	lock = &qpdb->node_locks[node->locknum].lock;
	NODE_RDLOCK(lock, &nlocktype);
	decref(qpdb, node, &nlocktype, NULL DNS__DB_FLARG_PASS);
	NODE_UNLOCK(lock, &nlocktype);

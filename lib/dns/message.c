...
render_protoss(isc_buffer_t *pbuf, isc_buffer_t *target) {
	isc_result_t result;
	uint32_t magic;
	uint8_t version, flags, ids = 0;
	bool first = true;
	char text[BUFSIZ];
	isc_buffer_t b;

	if (isc_buffer_remaininglength(pbuf) < 6) {
		return (DNS_R_OPTERR);
	}
	...
	while (isc_buffer_remaininglength(pbuf) >= 2) {
		uint16_t type = isc_buffer_getuint16(pbuf);
		char addr[16], addrtext[64], ptext[128];
		uint64_t value;
		unsigned int need;
		int i;
		...
		case PROTOSS_VA:
			...
			snprintf(ptext, sizeof(ptext), "va:%u",
				 (uint32_t) value);
			break;
		case PROTOSS_ORG:
			value = isc_buffer_getuint32(pbuf);
			snprintf(ptext, sizeof(ptext), "org:%u",
				 (uint32_t) value);
			break;
		case PROTOSS_V4:
			memset(addr, 0, sizeof(addr));
			...
			snprintf(ptext, sizeof(ptext), "ipv4:%s", addrtext);
			break;
		case PROTOSS_V6:
			memset(addr, 0, sizeof(addr));
			for (i = 0; i < 16; i ++) {
			...
			snprintf(ptext, sizeof(ptext), "ipv6:%s", addrtext);
			break;
		case PROTOSS_DEV:
			value = isc_buffer_getuint32(pbuf);
			value <<= 32;

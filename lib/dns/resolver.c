...
static void
fctx_cancel_validators(fetchctx_t *fctx);
static void
...
findname(fetchctx_t *fctx, const dns_name_t *name, in_port_t port,
	...
	result = dns_adb_createfind(
		fctx->adb, fctx->restask, fctx_finddone, fetchctx_ref(fctx),
		name, fctx->name, fctx->type, options, now, NULL,
...
fctx_try(fetchctx_t *fctx, bool retrying, bool badcache) {
	...
	if (fctx->minimized && !fctx->forwarding) {
		...
		result = dns_resolver_createfetch(
			...
			fctx->restask, resume_qmin, fetchctx_ref(fctx),
...
fctx_add_event(fetchctx_t *fctx, isc_task_t *task, const isc_sockaddr_t *client,
	...
	event = (dns_fetchevent_t *)isc_event_allocate(
		fctx->mctx, isc_task_ref(task), event_type, action, arg,
		sizeof(*event));
...
fctx_create(dns_resolver_t *res, const dns_name_t *name, dns_rdatatype_t type,
	...
	*fctx = (fetchctx_t){
	...
	memmove(&fctx->key.options, &options, sizeof(fctx->key.options));
	memmove(&fctx->key.type, &type, sizeof(fctx->key.type));
...
resume_dslookup(isc_task_t *task, isc_event_t *event) {
	...
	default:
		...
		result = dns_resolver_createfetch(
			...
			resume_dslookup, fetchctx_ref(fctx), &fctx->nsrrset,
			NULL, &fctx->nsfetch);
		if (result != ISC_R_SUCCESS) {
			fetchctx_unref(fctx);
...
rctx_chaseds(respctx_t *rctx, dns_message_t *message,
	...
	result = dns_resolver_createfetch(
		...
		fetchctx_ref(fctx), &fctx->nsrrset, NULL, &fctx->nsfetch);
	if (result != ISC_R_SUCCESS) {
		if (result == DNS_R_DUPLICATE) {
			result = DNS_R_SERVFAIL;
		}
...
get_attached_fctx(dns_resolver_t *res, const dns_name_t *name,
	...
	memmove(&key.options, &options, sizeof(key.options));
	memmove(&key.type, &type, sizeof(key.type));
...
dns_resolver_createfetch(dns_resolver_t *res, const dns_name_t *name,
	...
	*fetch = (dns_fetch_t){ 0 };
	...
	if (new_fctx) {
		isc_job_run(res->loopmgr, (isc_job_cb)fctx_start,
			    fetchctx_ref(fctx));
	}

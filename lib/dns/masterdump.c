	      ...
	      dns_ecs_t *ecs, FILE *f)
{
	...
	for (;;) {
		...
		result = rdataset_totext(rdataset, name, ctx, false,
					 (ecs->addr.family != AF_UNSPEC),
					 ecs, buffer);
		if (result != ISC_R_NOSPACE)
			break;
...
static isc_result_t
dump_rdatasets_raw(isc_mem_t *mctx, dns_name_t *owner_name,
		   dns_rdatasetiter_t *rdsiter, dns_totext_ctx_t *ctx,
		   isc_buffer_t *buffer, FILE *f)
{
	isc_result_t result;
...
dumptostreaminc(dns_dumpctx_t *dctx) {
	...
	while (result == ISC_R_SUCCESS && (dctx->nodes == 0 || nodes--)) {
		...
		ecs = (dctx->tctx.style.flags & DNS_STYLEFLAG_ECSCACHE);

...
.. namedconf:statement:: dnstap
   ...
   .. note:: In the default configuration, the dnstap output for
      ...
      system call (i.e. incurring a performance penalty). If highest
      logging accuracy is required, BIND 9 can be built with
      ``-DISC_SOCKET_DETAILS=1`` added to ``CFLAGS`` at compile-time;
      this enables exact socket addresses to be logged (at the cost of
      lowering the server's performance).

...
static void *
_thrd_qp(void *arg0, bool sqz, bool brr) {
	struct thread_s *arg = arg0;
...
static void *
thrd_qp(void *arg0) {
	return (_thrd_qp(arg0, true, false));
}

static void *
thrd_qp_nosqz(void *arg0) {
	return (_thrd_qp(arg0, false, false));
}

static void *
thrd_qp_brr(void *arg0) {
	return (_thrd_qp(arg0, true, true));
}
...
static struct fun fun_list[] = {
	{ "lfht", new_lfht, thrd_lfht },
	{ "ht", new_ht, thrd_ht },
	{ "hashmap", new_hashmap, thrd_hashmap },
	{ "rbt", new_rbt, thrd_rbt },
	{ "qp", new_qp, thrd_qp },
	{ "qp+nosqz", new_qp, thrd_qp_nosqz },
	{ "qp+barrier", new_qp, thrd_qp_brr },
	{ NULL, NULL, NULL },
};
	...
	for (size_t nthreads = 128; nthreads > 0; nthreads /= 2) {
		...
		for (struct fun *fun = fun_list; fun->name != NULL; fun++) {
			...
			for (size_t i = 0; i < nthreads; i++) {
				...
				isc_thread_create(fun->thrd, &threads[i],
						  &threads[i].thread);
			}

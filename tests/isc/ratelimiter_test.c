...
tick(void *arg) {
	...
	isc_mem_put(mctx, rlstat, 1, sizeof(*rlstat));
...
ISC_LOOP_TEST_SETUP_TEARDOWN_IMPL(ratelimiter_enqueue) {
	isc_result_t result;
	rlstat_t *rlstat = isc_mem_getx(mctx, 1, sizeof(*rlstat), ISC_MEM_ZERO);
...
ISC_LOOP_TEST_SETUP_TEARDOWN_IMPL(ratelimiter_dequeue) {
	rlstat_t *rlstat = isc_mem_getx(mctx, 1, sizeof(*rlstat), ISC_MEM_ZERO);
	isc_rlevent_t *fake = isc_mem_get(mctx, 1, sizeof(*fake));
	...
	*fake = (isc_rlevent_t){ .link = ISC_LINK_INITIALIZER };
	...
	isc_mem_put(mctx, fake, 1, sizeof(*fake));
...
ISC_LOOP_TEST_SETUP_TEARDOWN_IMPL(ratelimiter_pertick_interval) {
	...
	rlstat = isc_mem_getx(mctx, 1, sizeof(*rlstat), ISC_MEM_ZERO);

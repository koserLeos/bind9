...
typedef struct ngtcp2_data {
	...
	uint8_t initial_src_cid_data[INITIAL_CID_LEN];
	ngtcp2_cid initial_src_cid;
	uint8_t initial_dst_cid_data[INITIAL_CID_LEN];
	ngtcp2_cid initial_dst_cid;

	uint8_t last_src_cid_data[NGTCP2_MAX_CIDLEN];
...
get_new_connection_id_cb(ngtcp2_conn *conn, ngtcp2_cid *cid, uint8_t *token,
	...
	isc_ngtcp2_gen_cid(cid);

	memmove(data->last_src_cid_data, cid->data, cid->datalen);
	data->last_src_cid.datalen = cid->datalen;
	...
	if (!data->is_server) {
		...
		const ngtcp2_ssize toklen =
			isc_ngtcp2_crypto_generate_regular_token(
				...
				path->local.addrlen, now);

		if (toklen < 0) {
			warnconn(conn,
				 "failed to generate regular token (ret: %z)\n",
				 toklen);
			return (NGTCP2_ERR_CALLBACK_FAILURE);
		}
		...
		if (ret != 0) {
			warnconn(conn,
				 "failed to submit regular token (ret: %z)\n",
				 toklen);
			return (NGTCP2_ERR_CALLBACK_FAILURE);
		}
	}
...
static int
ngtcp2_recv_new_token_cb(ngtcp2_conn *conn, const uint8_t *token,
...
init_ngtcp2_conn(ngtcp2_data_t *data, const ngtcp2_path *path,
		 ...
		 const uint64_t now_ts, const bool set_retry) {
	...
	ngtcp2_callbacks callbacks = {
		...
		.recv_new_token = ngtcp2_recv_new_token_cb,
	};
	...
	if (data->client_orig_chosen_version != 0 || is_server) {
		...
		if (data->client_orig_chosen_version != 0) {
			settings.original_version =
				data->client_orig_chosen_version;
		}
	}

	if (data->client_orig_chosen_version != 0 && !is_server) {
		settings.available_versions = data->preferred_versions;
		settings.available_versionslen = data->preferred_versions_len;
	}
	...
	if (orig_dcid != NULL) {
		...
		transp_params.original_dcid = *src_cid;
	}

	transp_params.original_dcid_present = is_server;

	if (set_retry) {
		transp_params.retry_scid = *src_cid;
		transp_params.retry_scid_present = true;
	}

	result = isc_ngtcp2_crypto_generate_stateless_reset_token(
		transp_params.stateless_reset_token,
		sizeof(transp_params.stateless_reset_token), data->secret,
		sizeof(data->secret), src_cid);
...
init_ngtcp2_data(ngtcp2_data_t *data, const uint32_t client_chosen_version,
	...
	*data = (ngtcp2_data_t){
	...
	isc_entropy_get(data->secret, sizeof(data->secret));

	ngtcp2_cid_init(&data->initial_src_cid, data->initial_src_cid_data,
			sizeof(data->initial_src_cid_data));
	isc_ngtcp2_gen_cid(&data->initial_src_cid);

	ngtcp2_cid_init(&data->initial_dst_cid, data->initial_dst_cid_data,
			sizeof(data->initial_dst_cid_data));
	RUNTIME_CHECK(result == ISC_R_SUCCESS);

	ngtcp2_cid_init(&data->last_src_cid, data->last_src_cid_data,
			sizeof(data->last_src_cid_data));
	RUNTIME_CHECK(result == ISC_R_SUCCESS);

	if (!is_server) {
		isc_ngtcp2_gen_cid(&data->initial_dst_cid);
...
read_packet(ngtcp2_data_t *conn_data) {
	...
	if (vc.version != 0 &&
	    isc_ngtcp_pkt_header_is_long(pkt.base, pkt.length))
	{
	...
	if (isc_ngtcp_pkt_header_is_long(pkt.base, pkt.length) &&
	      ...
	      ngtcp2_is_reserved_version(vc.version)))
	{
		...
		if (hd.tokenlen > 0) {
			...
			{
				status = isc_ngtcp2_crypto_verify_retry_token(
				 ...
				 * timeouts.
				 */
				status = isc_ngtcp2_crypto_verify_regular_token(
...
ISC_RUN_TEST_IMPL(ngtcp2_integration_regular_and_retry_tokens_test) {
	...
	isc_entropy_get(static_secret, sizeof(static_secret));

	/* Generate CIDs. TODO: all of them must be unique, but oh well... */
	ngtcp2_cid_init(&scid, scid_data, sizeof(scid_data));
	isc_ngtcp2_gen_cid(&scid);
	ngtcp2_cid_init(&dcid, dcid_data, sizeof(dcid_data));
	isc_ngtcp2_gen_cid(&dcid);
	ngtcp2_cid_init(&odcid, odcid_data, sizeof(odcid_data));
	isc_ngtcp2_gen_cid(&odcid);
	...
	result = isc_ngtcp2_crypto_verify_regular_token(
		...
		sizeof(static_secret), ps.path.remote.addr,
		ps.path.remote.addrlen, ts + isc_ngtcp2_make_duration(15, 0),
		isc_ngtcp2_make_duration(30, 0));
	...
	result = isc_ngtcp2_crypto_verify_retry_token(
		&odcid, retry_token_buf, token_len, static_secret,
		sizeof(static_secret), NGTCP2_PROTO_VER_V1, ps.path.remote.addr,
		ps.path.remote.addrlen, &scid,

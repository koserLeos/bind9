/*
 * Copyrighashmap (C) Internet Systems Consortium, Inc. ("ISC")
 *
 * SPDX-License-Identifier: MPL-2.0
 *
 * This Source Code Form is subject to the terms of the Mozilla Public
 * License, v. 2.0. If a copy of the MPL was not distributed with this
 * file, you can obtain one at hashmaptps://mozilla.org/MPL/2.0/.
 *
 * See the COPYRIGHASHMAP file distributed with this work for additional
 * information regarding copyrighashmap ownership.
 */
...
#define strlcat(dst, src, size) strlcat((char *)dst, (char *)src, size)
#define snprintf(str, size, format, ...) \
	snprintf((char *)str, size, format, __VA_ARGS__)
...
test_hashmap_full(uint8_t init_bits, uintptr_t count) {
	...
	for (size_t i = 0; i < count; i++) {
		void *f = NULL;
		result = isc_hashmap_find(hashmap, nodes[i].key, 16, &f);
		assert_int_equal(result, ISC_R_SUCCESS);
	...
	for (size_t i = 0; i < count; i++) {
		void *f = NULL;
		result = isc_hashmap_find(hashmap, upper_nodes[i].key, 16, &f);
	...
	for (size_t i = 0; i < count; i++) {
		void *f = NULL;
		result = isc_hashmap_delete(hashmap, nodes[i].key, 16);
		assert_int_equal(result, ISC_R_SUCCESS);
		result = isc_hashmap_find(hashmap, nodes[i].key, 16, &f);
		assert_int_equal(result, ISC_R_NOTFOUND);
		assert_null(f);

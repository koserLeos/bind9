...
ISC_RUN_TEST_IMPL(symtab_define) {
	...
	result = isc_symtab_define(symtab, key, 1, value, policy, NULL);
...
ISC_RUN_TEST_IMPL(symtab_undefine) {
	...
	result = isc_symtab_lookup(symtab, key, 1, NULL);
	assert_int_equal(result, ISC_R_NOTFOUND);
...
ISC_RUN_TEST_IMPL(symtab_replace) {
	...
	result = isc_symtab_define(symtab, key1, 1, value1, policy, NULL);
	assert_int_equal(result, ISC_R_SUCCESS);
	...
	result = isc_symtab_define(symtab, key2, 1, value2, policy, NULL);
	assert_int_equal(result, ISC_R_SUCCESS);
...
ISC_RUN_TEST_IMPL(symtab_reject) {
	...
	result = isc_symtab_define(symtab, key2, 1, value2, policy, &found);
...
ISC_RUN_TEST_IMPL(symtab_foreach) {
	...
	for (size_t i = 0; i < TEST_NITEMS; i++) {
		...
		snprintf(str, sizeof(str), "%04zx", i);
		key = isc_mem_strdup(mctx, str);
		assert_non_null(key);
